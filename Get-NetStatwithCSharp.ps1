function Get-NetStatwithCSharp {
<#
.Synopsis
   Gets Netstat data from local machine.
.DESCRIPTION
   Gets Netstat data from local machine.This is mainlyfor Windows 2008 machines which is having PowerShell V2.0 installed.
.EXAMPLE
   Get-NetStat -TCPonly
.EXAMPLE
   Get-NetStat -UDPonly
...
#>
    ...
    Param
    (
        [Parameter(HelpMessage = "For getting only TCP connectionss")]
        [switch]$TCPonly,
        [Parameter(HelpMessage = "For getting only UDP connectionss")]
        [switch]$UDPonly
    )
...
public class Connection
{
    ...
    public string PIDName{ 
        get { 
            return (System.Diagnostics.Process.GetProcessById(_pid)).ProcessName;
        }
    }
}
"@
